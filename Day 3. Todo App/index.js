...
function onGetSavedNotes(){
    ...
    JSON.parse(notes).reverse().forEach(function(val,index){
        // Avoiding null objects in array
        // If objects exist then display notes
        if (val){
        // console.log(index)
        // console.log(val)
        noteHTML = noteHTML +
...
function deleteNotes(index){
    ...
    // This approach is only for practice
    // use splice method instead of this method
    delete myNotes.reverse()[index]// because we send index by reversing our array inorder to display last note on top
